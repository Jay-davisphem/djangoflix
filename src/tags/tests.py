...
class TaggedItemTestCase(TestCase):
    def setUp(self):
        self.ply_title = "Cool Playlist"
        self.ply_obj = Playlist.objects.create(title=self.ply_title)
        self.ply_obj.tags.add(TaggedItem(tag="new-tags"), bulk=False)
        self.ply_obj2 = Playlist.objects.create(title=self.ply_title)
        self.ply_obj2.tags.add(TaggedItem(tag="new-tags"), bulk=False)

    def test_content_type_is_not_null(self):
        with self.assertRaises(IntegrityError):
            tag = TaggedItem.objects.create(tag="my-new-tag")
    ...
    def test_create_via_content_type(self):
        ...
        tag_a = TaggedItem.objects.create(
            content_type=c_type, object_id=1, tag="my-new-tag"
        )
        self.assertIsNotNone(tag_a.pk)

        tag_b = TaggedItem.objects.create(
            content_type=c_type, object_id=1000, tag="my-new-tag2"
        )
    ...
    def test_create_via_app_loader_content_type(self):
        p_class = apps.get_model(app_label="playlists", model_name="Playlist")
        c_type = ContentType.objects.get_for_model(p_class)
        tag_a = TaggedItem.objects.create(
            content_type=c_type, object_id=1, tag="my-new-tag"
        )
        self.assertIsNotNone(tag_a.pk)
    ...
    def test_related_field_via_content_type(self):
        c_type = ContentType.objects.get_for_model(Playlist)
        tag_a = TaggedItem.objects.create(
            content_type=c_type, object_id=self.ply_obj.id, tag="my-new-tag"
        )
        self.assertIsNotNone(tag_a.pk)
